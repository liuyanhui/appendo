...
- **本地提醒** — 为条目设本地提醒，到点 appendo 自发通知（⏰），点按回到该条记录；支持贪睡、徽标、开机后补发；可设每天/每周重复
...
- 架构文档（现状，面向开发者）：[docs/architecture.md](./docs/architecture.md)
- 需求文档：[docs/specs.md](./docs/specs.md)
- 设计文档（历史变更史）：[docs/design.md](./docs/design.md)
...
- **旧数据中的 `---` 行与 `# Appendo` 开头行**：v1.2.1 起新写入的内容已受保护（视图可见、编辑不丢行）；升级前写入的此类行在条目视图中仍不可见，编辑该条目保存后这些行会被移除（「复制全部」走原文，不受影响）。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.2.0 | 2026-08-12 | 重复提醒（每天/每周）+ 手动输入设提醒/添加到日历入口 + 闹钟图标 |
