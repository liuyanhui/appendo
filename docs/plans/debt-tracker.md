...
- **TD-023 浅色强调色对比度低于 WCAG AA 正文标准（2026-09-17，v1.3 主题设计评审发现）**：实测 `Primary #2196F3` 对白底约 3.1:1（白字对蓝底同）——作按钮底色/组件边界达 AA 组件级 3:1，但作小号文字不达正文级 4.5:1。**决策（2026-09-17，用户拍板，设计 D1 方案 A）**：v1.3 深色新配色强制达标，浅色现状不动（避免主题版本被动变成浅色换色版本）；浅色文字用途若需达标，须引入文字专用深蓝（如 `#1976D2`，4.6:1）与容器蓝两档。优先级：低（组件级达标，仅小号文字偏弱）。
- **TD-024 realme 真机 adb 自动化验收基础不可靠（2026-09-17，v1.3 主题真机验收发现）**：①`screencap` 偶发返回旧帧（Activity 重建后截图仍显示重建前画面，与 uiautomator 树矛盾）——截图不能作为界面现状证据；②`uiautomator dump` 的控件 bounds 与屏幕实际像素疑似存在 ~140px 纵向偏移（菜单树内 y=288 起 vs 截图 y=430 起），`input tap` 实测与截图坐标系一致；③`SharedPreferences.apply()` 异步落盘在该机被延迟冻结（写入后 7 分钟不落盘、进程死即丢），`commit()` 同步落盘正常——主题偏好已改用 commit()（见 ThemePreferences KDoc），项目其他 apply() 路径历史验证正常、暂不跟随改动。**影响**：该机自动化验收（截图比对、脚本点击）不可依赖，交互验收应以人工为准、日志与 SP 文件为事实源。优先级：低（测试基建约束，已知即可）。
